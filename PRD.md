...
- 通用编辑器抽出能复用部分
- 豆包视觉规格见 §12（v1 仅做"快速模式"形态，思考模式与底部"快速 >"切换交互延后）
...
## 12. 豆包模板规格（一比一复刻 — Phase 6）

### 12.1 视觉规格（基于 doubao1/2.jpeg 测量 + 用户校核值）

**状态栏**：与 DeepSeek 完全一致（沿用同组件）

**顶栏**（结构与 DeepSeek 不同）：
- 高度 ~56px
- 左：≡ 菜单（24px，深灰）
- 中：标题（17px medium）+ 紧跟一个小 chevron-right（暗示可下拉）
- 标题下方：副标题"内容由豆包 AI 生成"，~12px，颜色 `#9A9A9A`
- 右：电话图标（**黑色描边**）+ 喇叭图标（**蓝色 `#0066FF` 填充**）

**用户气泡**：
- 背景 `#025AE1`（豆包深蓝主色）
- 文字 `#FFF`，字号 16px / 行高 1.5
- **四角均为圆角**（不像 DeepSeek 是非对称切角）
- padding 11×16，右对齐，max-width ~75%

**AI 回复卡片**（豆包独有结构）：
- 整段 AI 回复包在一张浅灰色圆角卡片里
- 背景 `#F7F7F7`
- 圆角 ~16px，padding 16px
- 卡片底部内含操作图标行
- 卡片内的 markdown 渲染样式与 DeepSeek 复用同一组件

**操作图标行**（卡片底部内）：
- 5 个图标：复制 / 喇叭 / 收藏 / 分享 / 重新生成
- 前 4 个左侧成组，重新生成单独右对齐
- 每个图标装在浅蓝小圆角方块容器：
  - 容器背景 `#E8EDF7`
  - 容器圆角 ~8px，~32×32px
...
**底部功能 toolbar**：
- 横向可滚动一排 pill 按钮
- 默认 pills：⚡快速 > / 🎙️录音纪要 / ✨AI 创作 / 🟦豆包 P 图 …（可在编辑器中增删/改名/调可见性）
- pill 样式：白底，1px 边框 `#E5E7EB`，圆角 999（完全圆角）
- padding 6×12，icon + 文字
- 整体 padding 10×12
- v1 不实现"快速 >"的模式切换交互，仅作视觉装饰

**输入栏**（与 DeepSeek 完全不同）：
- 单行 pill：📷 相机 icon + placeholder "发消息或按住说话…" + 🎙️ 语音 icon + ⊕ 加号
- **整体白底，无描边**
- 仅靠微微向下偏移的 box-shadow 区分（如 `0 -2px 8px rgba(0,0,0,0.04)`）
- 内圆角 ~22px
...
| 顶栏 | 标题 | string |
| 顶栏 | 标题尾 chevron | bool |
| 顶栏 | 副标题文本 | string，默认"内容由豆包 AI 生成" |
| 顶栏 | 显示菜单按钮（≡） | bool |
| 顶栏 | 显示电话图标 | bool |
| 顶栏 | 显示喇叭图标 | bool |
| 消息列表 | user / assistant 数组 | 增删拖拽 |
| 用户消息 | 文本 | string（纯文本） |
| AI 消息 | 主体内容 | string（markdown） |
| AI 消息 | 显示底部操作行 | bool |
| AI 消息 | 推荐追问列表 | string[]（每行一个问题；空数组=不显示） |
| 底部 toolbar | 显示 toolbar | bool |
| 底部 toolbar | pill 列表 | `{ id, icon, label, visible }[]` |
| 输入栏 | 显示 | bool |
| 输入栏 | placeholder | string |

### 12.3 Phase 6 限定（v1 暂不实现）

- 思考模式（与"快速模式"对应的另一种形态）
- 底部"快速 >" 点击展开模式选项的交互
- 这两项颜色/形状待后续视觉核对后补
