...
import type { DeepSeekState, DeepSeekMessage } from '../types'
import { newId } from '../defaults'
import { MessageCard } from './MessageCard'
import { PrimaryButton, GhostButton } from './fields'

export function MessagesTab({ state, onChange }: TemplateProps<DeepSeekState>) {
  const setMessages = (next: DeepSeekMessage[]) => onChange({ ...state, messages: next })
  ...
  const addAssistant = () => {
    setMessages([
      ...state.messages,
      {
        ...
        role: 'assistant',
        content: '',
        thinking: { enabled: false, seconds: 3, content: '', expanded: false },
        showActions: true,
      },
    ])
  }

  const updateMessage = (id: string, next: DeepSeekMessage) => {
